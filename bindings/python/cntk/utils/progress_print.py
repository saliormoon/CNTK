...
class TensorBoardProgressWriter(cntk_py.ProgressWriter):
    ...
    def on_write_training_update(self, samples, updates, aggregate_loss, aggregate_metric):
        # Override for ProgressWriter.on_write_training_update().
        self.write_value('minibatch/avg_loss', _avg(aggregate_loss, samples), updates[1])
        self.write_value('minibatch/avg_metric', _avg(aggregate_metric, samples), updates[1])
    ...
    def on_write_evaluation_summary(self, samples, updates, summaries, aggregate_metric, elapsed_milliseconds):
        ...
        if self.total_training_updates() != 0:
            ...
            self.write_value('minibatch/eval_avg_metric', avg_metric, self.total_training_updates())
        else:
            self.write_value('summary/eval_avg_metric', avg_metric, self.summaries)
